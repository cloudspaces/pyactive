...
from chord import Node, show, update
import copy

#Herencia de la clase Mensaje del fichero chord.py para anadir otros atributos que necesitamos en el Scribe
class ScribeMessage():
    
    def __init__(self,id, source, op, parent, text):
        self.id = id
        self.source = source
        self.operation = op         #Identificara si es un subscribe, unsubscribe o publish
        self.parent_node = parent   #Nos dira nuestro padre (util en el unsubscribe)
        self.text_message = text    #Llevara el mensaje a publicar (util en el publish)
        self.first_node = source    #Nos dice el primer nodo de todos (util en el subscribe)
    
    def get_source_node(self):
        return self.source_node

#Herencia de la clase Node del fichero chord.py para implementar los metodos forward y deliver para el Scribe
class ScribeNode(Node):
    ...
    _async = ['set_predecessor','remove_son','deliver','forward','set_parent', 'set_successor', 'show_finger_node',
               'stabilize', 'notify', 'fix_finger','set_my_topics', 'del_my_topics','leave']
    _ref = ['set_predecessor','remove_son','forward','set_parent','fail_stop_find_successor', 'get_predecessor', 
    ...
    def get_routing_topic_nodes(self):
        print 'get_routing', self.routing_topic_nodes
...
#    host = init_host(tcpconf)
    ...
    while index < num_nodes:
        try:
            if(nodes_h[index].join(nodes_h[0])):
                print "True"
            interval(3, update, nodes_h[index])
            index += 1
            retry = 0
            sleep(0.2)
        except TimeoutError:
    ...
    while (fin == False):
        ...
        if (op==1):     #Subscribe to a topic
            ...
            src.fail_stop_find_successor(ScribeMessage(key,src,'s',-1,''))
