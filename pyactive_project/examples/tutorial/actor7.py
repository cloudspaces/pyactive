...
class Web(object):
    _sync = {'list_files':20,'get_file':10}
...
def test():
    ...
    f1 = host.spawn_id('1', 'actor7', 'File', [])
    web = host.spawn_id('1', 'actor7', 'Web', [])
    web.remote_server(f1)
    load = host.spawn_id('1', 'actor7', 'Workload', [])
    load.remote_server(web)
    load2 = host.spawn_id('2', 'actor7', 'Workload', [])
